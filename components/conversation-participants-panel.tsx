...
export function ConversationParticipantsPanel({
  ...
  return (
    <section className="social-card space-y-4 p-4 sm:p-5">
      <div>
              ...
              <li key={entry.userId} className="fantasy-list-item flex items-center justify-between gap-3 rounded-xl px-3 py-2">
                <Link href={`/app/users/${entry.user.username}`} className="min-w-0 flex items-center gap-2">
                  <img
                    ...
                    className="h-8 w-8 rounded-full border border-slate-300/30 object-cover"
                  />
                  <span className="truncate text-sm font-medium text-slate-100">{entry.user.displayName}</span>
                  <span className="truncate text-xs text-slate-400">@{entry.user.username}</span>
                </Link>
        ...
        <div className="space-y-2">
          ...
          {isInvitePanelOpen ? (
            <form onSubmit={inviteParticipant} className="space-y-2">
              <label htmlFor="invite-user" className="text-xs font-medium uppercase tracking-wide text-slate-400">
              ...
              {availableInvitees.length === 0 ? <p className="text-xs text-slate-400">Everyone is already in this conversation.</p> : null}
            </form>
          ) : null}
        </div>
      ) : null}
